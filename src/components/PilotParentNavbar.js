...
const PilotParentNavbar = ({ handleInteraction, isCollapsed, onToggleCollapse }) => {
  const { user } = useAuth()
  ...
  // Simplified navigation sections for pilot parent users
  const pilotParentNavSections = [
    {
      id: 'home',
      label: 'Home',
      icon: HomeIcon,
    ...
    >
      <Box sx={{ flex: 1, py: 'var(--spacing-4)' }}>
        ...
        >
          ...
          {/* Pilot parent nav sections */}
          {pilotParentNavSections.map((section) => (
...
export default withAuthCheck(PilotParentNavbar) 