-- Fix any remaining issues with the household tables schema

-- First, check if the tables exist and recreate them if needed
DO $$
BEGIN
    -- Check if households table exists
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'households') THEN
    ...
    -- Check if household_invitations table exists
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'household_invitations') THEN
